...
def main():
    ...
    while running:
        ...
        if auto_spawn_obs and obs_timer > 5.0 and random.random() < 0.3:
            lane_idx = int((player.x - ROAD_X_START) / LANE_WIDTH)
            lane_idx = max(0, min(2, lane_idx)) # Spawn in player lane to force AEB/ESA
            lane_x = ROAD_X_START + lane_idx * LANE_WIDTH + (LANE_WIDTH - 40)//2
        ...
        for i in range(4):
            ...
            if i == 0 or i == 3:
                ...
                # Solid lines for inner lanes as requested
                pygame.draw.line(screen, COLOR_LANE_MARKER, (x, 0), (x, SCREEN_HEIGHT), 2)
